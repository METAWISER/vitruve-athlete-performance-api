...
export const register = (router: Hono): void => {
  ...
  const metricsAggregateService = new AthleteMetricsAggregateService(
    metricsRepository,
    athleteRepository
  );
  const httpResponse = new HttpResponse();

  const athleteMetricsAggregateController =
    new AthleteMetricsAggregateController(
  ...
  router.get(
    ...
    validateJWT,
    async (c) => await athleteMetricsAggregateController.run(c)
  );
};
